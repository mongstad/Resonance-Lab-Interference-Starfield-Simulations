...
with tab1:
    ...
    if preset != "Custom":
        ...
        modes = st.sidebar.slider("Modes", 1, 60, 18)
        types = st.sidebar.multiselect("Types", ["plane", "radial", "spiral"],
                                       default=["plane", "radial", "spiral"])
        extent = st.sidebar.slider("Extent", 1.0, 20.0, 3.0)
        kx_range = st.sidebar.slider("kx range", 0.1, 40.0, (1.0, 8.0))
        ky_range = st.sidebar.slider("ky range", 0.1, 40.0, (1.0, 8.0))
        kr_range = st.sidebar.slider("kr range", 0.1, 40.0, (1.0, 8.0))
        ktheta_range = st.sidebar.slider("kθ range", 0.1, 40.0, (1.0, 8.0))
        amp_range = st.sidebar.slider("Amplitude range", 0.1, 3.0, (0.6, 1.2))
        phase_range = st.sidebar.slider("Phase range", 0.0, 6.283, (0.0, 6.283))
    ...
    frames = st.sidebar.slider("Frames", 30, 2000, 240, step=10)
    fps = st.sidebar.slider("FPS", 5, 60, 30)
    time_scale = st.sidebar.slider("Time evolution scale", 0.0, 2.0, 0.3, step=0.05)
...
with tab2:
    ...
    with zcol2:
        base_extent = st.number_input("Initial extent", min_value=1.0, max_value=40.0,
                                      value=4.0, step=0.5, key="zoom_extent")
    with zcol3:
        base_modes = st.number_input("Modes", min_value=1, max_value=60,
    ...
    with zc3:
        zoom_factor = st.number_input("Zoom factor (>1 = deeper)", value=2.0,
                                      min_value=1.01, max_value=10.0, step=0.1, key="zoom_factor")
    ...
    if st.button("Generate Base"):
        ...
        X, Y, Z = generate_field(size=base_size, extent=st.session_state.zoom_extent,
                                 modes=int(base_modes), mode_types=ztypes, rng=np.random.RandomState(None))
        render_field(X, Y, Z, zout, cmap=zcmap, contours=zcontours, colorbar=False, dpi=180)
        st.image(zout, caption=f"Extent {st.session_state.zoom_extent:.3f}", use_container_width=True)
    ...
    if st.button("Zoom Deeper"):
        ...
        X, Y, Z = generate_field(size=base_size, extent=st.session_state.zoom_extent,
                                 modes=int(base_modes), mode_types=ztypes, rng=np.random.RandomState(None))
        render_field(X, Y, Z, zout, cmap=zcmap, contours=zcontours, colorbar=False, dpi=180)
        st.image(zout, caption=f"Zoom Level Extent {st.session_state.zoom_extent:.3f}", use_container_width=True)
